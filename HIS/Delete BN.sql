...
DELETE FROM HIS_SERVICE_ACCESSORY
WHERE HIS_CONSUMPTIONGROUP_ID IN (SELECT HIS_CONSUMPTIONGROUP_ID FROM HIS_CONSUMPTIONGROUP cg WHERE cg.HIS_PATIENTHISTORY_ID IN (SELECT HIS_PatientHistory_ID FROM HIS_PatientHistory WHERE Name like '%TEST%'));
