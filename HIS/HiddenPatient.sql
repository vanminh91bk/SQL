...
WHERE HIS_ServiceType = 'Medicine' AND
        HIS_Service_Record_ID IN (
            ...
            WHERE HIS_PatientHistory_ID IN (SELECT HIS_PatientHistory_ID FROM HIS_PatientHistory WHERE HIS_PatientHistory_ID in (2045778))
        ) ;
...
Update HIS_SERVICE_ACCESSORY set IsActive = 'N'
WHERE HIS_CONSUMPTIONGROUP_ID IN (SELECT HIS_CONSUMPTIONGROUP_ID FROM HIS_CONSUMPTIONGROUP cg WHERE cg.HIS_PATIENTHISTORY_ID IN (SELECT HIS_PatientHistory_ID FROM HIS_PatientHistory WHERE HIS_PatientHistory_ID in (2045778)));
